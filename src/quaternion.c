...
quaternion_t quaternion_new (double w, double x, double y, double z) {
	quaternion_t toret = {w, x, y, z};
	return toret;
}
...
double quaternion_dot (quaternion_t a, quaternion_t b) {
...
// }}}
// Metric between two rotations {{{

double quaternion_metric (quaternion_t a, quaternion_t b) {
	a = quaternion_scale(a, 1.0 / quaternion_mag(a));
	b = quaternion_scale(b, 1.0 / quaternion_mag(b));
	double dp = quaternion_dot(a, b);
	if (dp > 1.0) {
		dp = 1.0;
	}
	return acos(fabs(dp));
